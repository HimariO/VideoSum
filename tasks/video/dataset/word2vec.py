...
def build_dataset(words):
    ...
    print(colored(len(C), color='yellow'))
    dictionary = dict()
    for (word, _) in count:
...
# print('EOS' in dictionary.keys())
del words  # Hint to reduce memory.
print('Most common words (+UNK)', count[:5])
print('Sample data', data[:10], [reverse_dictionary[i] for i in
      data[:10]])
...
with open('MSR_enW2V_dict.csv', 'w') as dict_file:
        writer = csv.DictWriter(dict_file, ['word', 'id'])
        writer.writeheader()

        for key in reverse_dictionary.keys():
            writer.writerow({'word': reverse_dictionary[key], 'id': key})

np.save('MSR_enW2V.npy', final_embeddings)
